...
void CMasternodeSync::GetNextAsset()
{
    ...
    case (MASTERNODE_SYNC_FAILED): // should never be used here actually, use Reset() instead
        ...
        LogPrintf("CMasternodeSync::GetNextAsset - Spork\n");
    ...
    case (MASTERNODE_SYNC_MNW):
        LogPrintf("CMasternodeSync::GetNextAsset - Budget\n");
...
void CMasternodeSync::Process()
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes) {
        ...
        if (RequestedMasternodeAssets == MASTERNODE_SYNC_SPORKS) {
            if (lastSporks > 0 && lastSporks < GetTime() - MASTERNODE_SYNC_TIMEOUT * 2 && RequestedMasternodeAttempt >= MASTERNODE_SYNC_THRESHOLD) {
                GetNextAsset();
                return;
            }
