...
std::list<std::string> ZBLASTControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBLASTControlDialog::listMints;

ZBLASTControlDialog::ZBLASTControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBLASTControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBLASTControlDialog::~ZBLASTControlDialog()
{
    delete ui;
}

void ZBLASTControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBLASTControlDialog::updateList()
{
...
void ZBLASTControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBLASTControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBLAST_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBLASTControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBLASTControlDialog::GetSelectedMints()
{
...
void ZBLASTControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
