                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBLAST ought to be enough for anybody." - Bill Gates, 2017
    ui->zBLASTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBLASTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBLAST->setEnabled(false);
        ui->pushButtonMintzBLAST->setToolTip(tr("zBLAST is currently disabled due to maintenance."));

        ui->pushButtonSpendzBLAST->setEnabled(false);
        ui->pushButtonSpendzBLAST->setToolTip(tr("zBLAST is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBLASTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBLAST_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBLAST is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBLAST...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBLAST in ") +
...
void PrivacyDialog::on_pushButtonSpendzBLAST_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBLAST is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBLAST
        sendzBLAST();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBLAST
    sendzBLAST();
}

void PrivacyDialog::on_pushButtonZBLASTControl_clicked()
{
    ZBLASTControlDialog* zBLASTControl = new ZBLASTControlDialog(this);
    zBLASTControl->setModel(walletModel);
    zBLASTControl->exec();
}

void PrivacyDialog::setZBLASTControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBLASTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBLAST()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BlastX Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBLASTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBLAST is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BLAST </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBLAST</b>";
    ...
    // use mints from zBLAST selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBLASTControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBLASTControlDialog::GetSelectedMints();
    }
    ...
    ZBLASTControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBLAST Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BLAST, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBLAST Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBLAST </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBLAST "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBLAST "));
    ui->labelzBLASTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBLASTSyncStatus->setVisible(fShow);
}
