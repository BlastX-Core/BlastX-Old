...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BLAST and zBLAST money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"),  32009,  32008));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, blastx, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BlastX specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235: 32009"));
    ...
    strUsage += HelpMessageOpt("-backupzciv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBLAST minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeblastxamount=<n>     " + strprintf(_("Keep N BLAST anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"),  31008, 31006));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The BlastX Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZBLASTMinted();
                        RecalculateZBLASTSpent();
                    }
                    RecalculateBLASTSupply(1);
                }
                ...
                // BlastX: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZBLASTBackups = GetBoolArg("-backupzciv", true);
        pwalletMain->setZBLASTAutoBackups(fEnableZBLASTBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeBlastXAmount = GetArg("-anonymizeblastxamount", 0);
//    if (nAnonymizeBlastXAmount > 999999) nAnonymizeBlastXAmount = 999999;
//    if (nAnonymizeBlastXAmount < 2) nAnonymizeBlastXAmount = 2;
    ...
    LogPrintf("Anonymize BlastX Amount %d\n", nAnonymizeBlastXAmount);
    ...
    /* Denominations
       ...
       For example:
       1BLAST+1000 == (.1BLAST+100)*10
       10BLAST+10000 == (1BLAST+1000)*10
    */
