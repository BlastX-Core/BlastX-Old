...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BLAST at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBLAST send
enum ZerocoinSpendStatus {
    ZBLAST_SPEND_OKAY = 0,                            // No error
    ZBLAST_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBLAST_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBLAST_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBLAST_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBLAST_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBLAST_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBLAST_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBLAST_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBLAST_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBLAST_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBLAST_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBLAST_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBLAST_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBLAST_SPENT_USED_ZBLAST = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZBLASTAutoBackups(bool fEnabled)
    {
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == ActiveCollateral()) continue; // do not count MN-like outputs
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == ActiveCollateral()) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
